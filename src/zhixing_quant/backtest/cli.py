...
退出码：跑完并登记 0；跑完了而**这一跑不成立**（台账 `void`：三档全无回合，票池或区间不对）1；
根本没开始（缺票池、假设表读不出、dataset 不是分钟线、区间颠倒、策略路径不存在、快照读不出、
日线因子没落盘）2；`--task` 指向一个不存在的任务 3。**回测结论是负的也退出 0**——"这套信号不
赚钱"是一次成功的实验，不是一次失败的运行（07 §一：宁可不用是合法结论）。

0 与 1 那条线为什么划在 void 而不是划在 fail：`report.status()` 自己就把 void 判成"不是失败，
是作废"——它说的是什么都没测出来，不是一个结论，也就没资格享受"结论一律 0"那条豁免。而这一层
越诚实越拦不住人：第七节那句缺口披露写在报告页里，页要有人去读；`&&`、`xargs` 与定时器只看退出
码。一跑作废发 0，等于把坑 #29 那整晚留给了下一个读报告的人。
"""
...
def _execute(
    ...
    *,
    ...
    if verdict == page.VOID:
        print(
            "这一跑**不成立**（作废）：三档成本下都没有一个回合，票池或区间不对——"
            "退出码 1 说的是这次运行，不是这套信号的结论（负结论照样退出 0）。"
        )
        return 1
    return 0
