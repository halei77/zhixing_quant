...
- `volume`：量或额相对偏差超过容差。同一批全量实测：330 条、165 个 (票,天,周期) 组合、
  ...
  但**尾差不是收盘集合竞价**：被查的 3,895 个组合每个都是满根（5min 48、30min 8、60min 4），
  末根 15:00 全在盘上，且 3,910 个组合里末根零成交的是 0 根。盘上另有 15 个组合不是满根——
  各 dataset 的首日只落了尾盘 2 根，拿去跟整天对出 -48%~-96.9%，那是扫描器自己造的偏差而非
  源的，`tools/reconcile_history.py` 因此从第二天算起。真正指向源的是另一件事：**同一天的缺口
  在三个周期间互不相等**（2026-09-14 的 000001：5min 少 2,468,100 股、30min 少 5,851,400 股、
  60min 一分不少）。能在盘中定位差额落在哪根的那一天（2026-09-14，只有 5min 覆盖满的日子才
  定位得了），差额在上午 11:00/11:30 那根粗K上——同一段时间 5 分钟侧合计 3,385,300 股，
  30 分钟侧那根只有 2,000 股。所以这条抓到的是**源自己的几本账不一致**，不是我们的聚合少加了。
"""
