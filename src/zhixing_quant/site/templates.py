...
def parse(data: Mapping[str, Any]) -> Config:
    ...
    templates = tuple(_template(row, index) for index, row in enumerate(_rows(data)))
...
def _template(row: object, index: int) -> Template:
    where = f"第 {index} 条模板"
    ...
    template = Template(
        ...
        data=_selections(_require(data, "data", where=where), where=where),
