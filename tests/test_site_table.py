...
def test_reference_headers_cover_every_selectable_field() -> None:
    ...
    forecast / news / analyst_rating 例外：事件型表走自己的固定表头（公告日/报告期/…、
    公告日/挂网时刻/…、发布日/券商/评级/目标价…），`VALUATION_HEADERS.get(c, c)` 的兜底
    路径它们走不到——硬要登记是一份没人用的重复清单。
    """
    covered = set(table.VALUATION_HEADERS) - {"time"}
    for dataset, fields in templates.TABLE_DATASETS.items():
        if dataset in ("forecast", "news", "analyst_rating"):
            continue
