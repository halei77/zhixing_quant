...
def test_backtest_artifacts_are_archived_under_the_data_root(tmp_path: Path) -> None:
    """09 §五 与 02 §六 都写了 `backtests/`：大件在盘上，台账只存路径，所以根必须在配置里。"""
    assert config.backtests_dir({"ZX_DATA_ROOT": str(tmp_path)}) == tmp_path / "backtests"
