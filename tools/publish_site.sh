...
# `rsync -az --delete .../data/news .../golden/ aliyun:/opt/zhixing_data/`——目标少了 `/data`
# 且 `--delete` 开着，`/opt/zhixing_data/data/` 被整目录删掉，生产全挂几分钟。本机数据未受损，
# 用本脚本全量发回即恢复。本脚本是唯一有清单、有健康检查、被跑过无数次的那条路。
set -euo pipefail
cd "$(dirname "$0")/.."
...
for t in daily_basic forecast stk_limit fina_audit stk_holdernumber index_daily report_rc fina_indicator news; do
