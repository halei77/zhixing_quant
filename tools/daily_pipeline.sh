#!/usr/bin/env bash
# 每日盘后流水线：采集（zx-daily 全市场 + zx-minute 5/30/60 全市场 + zx-minute 1 分试采池）→ 发布。
...
#
# zx-minute --periods 5,30,60 是 ADR-0021 决定 3 + ADR-0024 决定 3 的落点：sina 无窗口参数、
# 每次固定回最近 1970 根，**主源滑出 tail 的日子就得靠 ngw 兜底重跑才补得回**（ADR-0024 决定 4
# 作废了"漏抓即永久缺失"，但可补≠自动补）——所以三周期是每日必采。范围 = 全市场三周期
# （ADR-0021 决定 1 的全市场 + ADR-0024 决定 3 把 5/30 从"暂不采"提为"每日采"，成本 ≈+3h/日、
# 盘 ≈+1GB，实测服务器 34G 空闲扛得住）。三周期 sina 抓空/抓错由 ngw 同周期降级（预算 300/轮）。
#
...
LOG="${ZX_DATA_ROOT:-/home/lei/zhixing_data}/reports/daily_pipeline.log"
{
  ...
  .venv/bin/zx-minute --periods 5,30,60 --limit 6000 --attempts 3 --breaker 80
  rc_minute=$?
  echo "----- zx-minute 5/30/60 rc=$rc_minute -----"
